...
import artisynth.core.mechmodels.RigidCompositeBody;
...
/*/ TODO:
 * Model organization:
 *  - use enum or similar for handling name
 *  - handle the muscle bundles + exciters more simply
 *  - gather CSA data for all muscles, base all muscle parameters off of anatomical data
 *  - create attachment surfaces to replace all dependencies on meshes
 *  - "ligaments" grouping is incomplete --> many larynx "muscles" are ligaments...
 *  - create rbConnector grouping
 *  
 * Model functionality:
 *  - Tongue-face attachments?
 *  - implement new larynx model; consider re-scaling?
 *  - larynx collisions cause errors
 *  - appropriate muscle forces for the pharynx
 *  - load the spine + muscles in a consistent manner
 *  - make external muscles symmetrical (should be nearly so)
 *  - add support for a symmetrical simulation
 *  
 * Misc:
 *  - get the medical images for PB, have those loadable?
 *  - control panel: add switches to de-activate models as needed
 *  - why do control panels take so long to build?
 *  - have no dependencies outside of artisynth_core and maspack
 *  - is there a simple BV method for finding all nodes w/in a radius? Useful for attachments and marker radius alloc
 *  - organization: SoftTissue, Bone, Ligament, Muscles
/*/

public class FrankModel2 extends RootModel  
{
    public MechModel mechModel = new MechModel();
    
    public String modelDir   = ArtisynthPath.getSrcRelativePath (FrankModel2.class, "");
    ...
    SkinMeshBody airway_tube_clipped = new SkinMeshBody();
    //0714Cyd
    SkinMeshBody airway_tube_clipped_manual = new SkinMeshBody(); 
    //---0714Cyd
    //0807Cyd
    SkinMeshBody airway_complete_tube_clipped_processed_0728 = new SkinMeshBody(); 
    SkinMeshBody airway_complete_tube_clipped_processed_0807 = new SkinMeshBody(); 
    //---0807Cyd
...
//    NodeCaching nodeCaching = NodeCaching.Ignore;
    ...
    protected boolean saveGeometries = true;
    //------------20260702Cyd
    ...
    public void build (String[] args) throws IOException 
    {
        ...
        loadRigidBody(jaw,          "jaw",          geometryDir, "jaw.vtk",         3470.0);
        loadRigidBody(maxilla,      "maxilla",      geometryDir, "maxilla.vtk",     1900.0);
        maxilla.setDynamic(false);
        loadRigidBody(hyoid,        "hyoid",        geometryDir, "hyoid.vtk",       1900.0);
        loadRigidBody(thyroid,      "thyroid",      geometryDir, "thyroid.vtk",     1900.0);
        loadRigidBody(cricoid,      "cricoid",      geometryDir, "cricoid.vtk",     1900.0);
        loadRigidBody(epiglottis,   "epiglottis",   geometryDir, "epiglottis.vtk",  1900.0);
        loadRigidBody(cranium,      "cranium",      geometryDir, "cranium.vtk",     1900.0);
        ...
        loadRigidBody(arytenoid_L,  "arytenoid_L",  geometryDir, "arytenoid_L.vtk", 1900.0);
        loadRigidBody(arytenoid_R,  "arytenoid_R",  geometryDir, "arytenoid_R.vtk", 1900.0);
//        loadRigidBody(skull,        "skull",        geometryDir, "badinskull.obj", 1900.0);
//        skull.setDynamic(false);
        //loadRigidBody(palateRB, "palateRB", geometryDir, "palateRB.vtk", 1900.0);
...
//        palateMaterial.setC10 (K/10.0);                                   // Pa  [Buchaillard 2009],[Duck 1990]
        ...
        String faceMeshName = "face";
        GenericModel.loadFemMesh_VTK (face, geometryDir, faceMeshName);
        fems.add(face);
        face.setName ("face");
        MooneyRivlinMaterial faceMaterial = new MooneyRivlinMaterial();
        faceMaterial.setBulkModulus(25000.0);
        faceMaterial.setBulkPotential(BulkPotential.QUADRATIC);
        faceMaterial.setC10(2500.0);
        faceMaterial.setC01(0.0);
        faceMaterial.setC11(0.0);
        faceMaterial.setC20(1175.0);
        faceMaterial.setC02(0.0);
        faceMaterial.setJLimit(0.0);
        face.setMaterial (faceMaterial);
        //face.setMaterial ( new LinearMaterial(15000.0, 0.49) );
        //setLinearFaceRegion(faceMaterial, new LinearMaterial(15000.0, 0.49) );
        face.setIncompressible (IncompMethod.OFF);
        face.setDensity (1040.0);
        face.setParticleDamping(10.0);
        face.setStiffnessDamping(0.03);
        
        stopwatch.checkpoint("Loading FEMs: face loaded");
...
//                node.setDynamic(false);
        ...
        // face
        applyFemStaticNodes(face, faceMeshName);        // TODO: mesh dependent
        attachFemToRigidBody(face, jaw, faceMeshName);  // TODO: mesh dependent
        ...
        //addAirwaySkin(airway_tube_clipped, "airway_tube_clipped", "airway_complete_tube_clipped.ply");
        //0714Cyd
        //addAirwaySkin(airway_tube_clipped_manual, "airway_tube_clipped_manual", "airway_complete_tube_clipped_manul_seclection.ply"); 
        //---0714Cyd
        //0807Cyd
        // airway_complete_tube_clipped_processed_0728 = addAirwaySkin(
        //         "airway_complete_tube_clipped_processed_0728", 
        //         "airway_complete_tube_clipped_processed_0728.ply"
        //     );
        airway_complete_tube_clipped_processed_0807 = addAirwaySkin(
                "airway_complete_tube_clipped_processed_0807", 
                "airway_complete_tube_clipped_processed_0807.ply"
            );
        //---0807Cyd
        saveAirwaySTL();
    ...
    // add eyeballs
    protected void addEyes() {
       try {
          WavefrontReader wfr = new WavefrontReader(ArtisynthPath.getSrcRelativeFile(FrankModel2.class, "geometry/eye/eyeball.obj"));
          wfr.parse();

          RigidCompositeBody left_eye = new RigidCompositeBody("left_eye");
          RigidCompositeBody right_eye = new RigidCompositeBody("right_eye");

          for (String group : wfr.getGroupNames()) {
             PolygonalMesh mesh = new PolygonalMesh();
             wfr.setMesh(mesh, group);
             if (mesh.numVertices() > 0) {
                mesh.scale(0.01);
                left_eye.addMesh(mesh);
                mesh = new PolygonalMesh();
                wfr.setMesh(mesh, group);
                mesh.scale(0.01);
                right_eye.addMesh(mesh);
             }
          }
          left_eye.setMass(1e-10);  // barely any mass
          right_eye.setMass(1e-10); // barely any mass
          
          left_eye.setPose(new RigidTransform3d(
             new Vector3d(0.08068897206111518, -0.029236643669426056, 0.15878865576676257),
             new AxisAngle(-0.025338678344634647, -0.9993577451340907, -0.02533867834463459, 1.571438787950616)
             ));
          right_eye.setPose(new RigidTransform3d(
             new Vector3d(0.08014276724785206, 0.028536213808294283, 0.15878865576676257),
             new AxisAngle(-0.05145066905929765, -0.9973493155894283, -0.05145066905929762, 1.573450527373392)
             ));
          
          rbs.add(left_eye);
          rbs.add(right_eye);

          FrameAttachment eyeAttach = cranium.createFrameAttachment(left_eye, left_eye.getPose());
          mechModel.addAttachment(eyeAttach);
          
          eyeAttach = cranium.createFrameAttachment(right_eye, right_eye.getPose());
          mechModel.addAttachment(eyeAttach);
...
//            GenericModel.renderFemMarkers    (fem, 0.7*scaling, new Color(0.6f, 0.0f, 0.6f));
        ...
        for (SkinMeshBody aw : new SkinMeshBody[]{airway_complete_tube_clipped_processed_0807})
        {
...
//            nStep++;
        ...
        if (saveGeometries == true)
        {
            try
            {
                ...
                //20260702Cyd
                //PlyWriter.writeMesh(String.format("%sfrank_%s_%05.3f.ply", geomDir, airway_tube_clipped.getName(), t0), airway_tube_clipped.getSurfaceMesh());
                //------------20260702Cyd
                //20260714Cyd
                //PlyWriter.writeMesh(String.format("%sfrank_%s_%05.3f.ply", geomDir, airway_tube_clipped_manual.getName(), t0), airway_tube_clipped_manual.getSurfaceMesh());  
                //------------20260714Cyd
                //0807Cyd
                //PlyWriter.writeMesh(String.format("%sfrank_%s_%05.3f.ply", geomDir, airway_complete_tube_clipped_processed_0728.getName(), t0), airway_complete_tube_clipped_processed_0728.getSurfaceMesh());
                PlyWriter.writeMesh(String.format("%sfrank_%s_%05.3f.ply", geomDir, airway_complete_tube_clipped_processed_0807.getName(), t0), airway_complete_tube_clipped_processed_0807.getSurfaceMesh());
                //---0807Cyd


                //20260706Cyd .stl  output
                //((PolygonalMesh) airway.getSurfaceMesh()).write(
                //    new File(String.format("%sfrank_%s_%05.3f.stl", geomDir, airway.getName(), t0)), "%.8g", false);
                //((PolygonalMesh) airway_tube.getSurfaceMesh()).write(
                //    new File(String.format("%sfrank_%s_%05.3f.stl", geomDir, airway_tube.getName(), t0)), "%.8g", false);
                //((PolygonalMesh) airway_tube_clipped.getSurfaceMesh()).write(
                //    new File(String.format("%sfrank_%s_%05.3f.stl", geomDir, airway_tube_clipped.getName(), t0)), "%.8g", false);
                // --------------------20260706Cyd .stl  output
                //20260714Cyd .stl  output
                //((PolygonalMesh) airway_tube_clipped_manual.getSurfaceMesh()).write(
                //    new File(String.format("%sfrank_%s_%05.3f.stl", geomDir, airway_tube_clipped_manual.getName(), t0)), "%.8g", false); 
                //--------------------20260714Cyd .stl  output
                //0807Cyd .stl  output
                //((PolygonalMesh) airway_complete_tube_clipped_processed_0728.getSurfaceMesh()).write(
                //   new File(String.format("%sfrank_%s_%05.3f.stl", geomDir, airway_complete_tube_clipped_processed_0728.getName(), t0)), "%.8g", false);
                ((PolygonalMesh) airway_complete_tube_clipped_processed_0807.getSurfaceMesh()).write(
                    new File(String.format("%sfrank_%s_%05.3f.stl", geomDir, airway_complete_tube_clipped_processed_0807.getName(), t0)), "%.8g", false);
                //--------------------0807Cyd .stl  output
            ...
            {System.err.println("Export failed at t0=" + t0 + ": " + e.getMessage());
                e.printStackTrace();}
        }
    ...
    {
        items.add (GuiUtils.createMenuItem (this, "speech_a_stavness2012", ""));
        items.add (GuiUtils.createMenuItem (this, "speech_a_0723Cyd", ""));
        items.add (GuiUtils.createMenuItem (this, "speech_i", ""));
        items.add (GuiUtils.createMenuItem (this, "speech_i_stavness_0720Cyd", ""));
        items.add (GuiUtils.createMenuItem (this, "speech_i_partially_stavness_0720Cyd", ""));
        items.add (GuiUtils.createMenuItem (this, "speech_u", ""));
        items.add (GuiUtils.createMenuItem (this, "speech_aiu", ""));
        //20260715Cyd
        items.add (GuiUtils.createMenuItem (this, "speech_a_wu2014_0715Cyd", ""));  
        items.add (GuiUtils.createMenuItem (this, "speech_a_payan2009", "")); 
        //------20260715Cyd
        //20260901Cyd
        items.add (GuiUtils.createMenuItem (this, "speech_r_stavness_0901Cyd", ""));
        //------20260901Cyd
    ...
    public void actionPerformed (ActionEvent event) 
    {
        ...
        if      (cmd.equals ("speech_a_stavness2012"))
            FrankActivations.probe_snd_a(this, mechModel, allExciters);
        else if (cmd.equals ("speech_a_0723Cyd"))
            FrankActivations.probe_snd_a_0723Cyd(this, mechModel, allExciters);
        
        else if (cmd.equals ("speech_i"))
            FrankActivations.probe_snd_i(this, mechModel, allExciters);
        else if (cmd.equals ("speech_i_stavness_0720Cyd"))
            FrankActivations.probe_snd_i_stavness_0720Cyd(this, mechModel, allExciters);
        else if (cmd.equals ("speech_i_partially_stavness_0720Cyd"))
            FrankActivations.probe_snd_i_partially_stavness_0720Cyd(this, mechModel, allExciters);
        ...
        //20260715Cyd
        else if (cmd.equals ("speech_a_payan2009"))
            FrankActivations.probe_snd_a_payan2009(this, mechModel, tongue);
        else if (cmd.equals ("speech_a_wu2014_0715Cyd"))
            FrankActivations.probe_snd_a_wu2014_0715Cyd(this, mechModel, tongue.getMuscleBundles(), externalMuscles, tongue, larynx);
        //------20260715Cyd
        //20260901Cyd
        else if (cmd.equals ("speech_r_stavness_0901Cyd"))
            FrankActivations.probe_snd_r_stavness_0901Cyd(this, mechModel, allExciters);
        //------20260901Cyd
...
//            face.setIncompressible (IncompMethod.OFF);
    ...
    // Extract the airway mesh as a stl file.
    public void saveAirwaySTL() throws IOException {
       String outputPath = geometryDir + "airway_tube_rest.stl";
       PolygonalMesh mesh = (PolygonalMesh) airway_tube.getMesh();
       mesh.write(new File(outputPath), "%.8g", false);
       System.out.println("Airway tube mesh saved to: " + outputPath);
   }
