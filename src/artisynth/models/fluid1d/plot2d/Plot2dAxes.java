...
/**
 * @author petera, though much of this code has been borrowed from NumericProbePanel.
 */

public class Plot2dAxes extends JPanel 
{

   double[] xLim = {0.0, 1.0};
   double[] yLim = {0.0, 1.0};
   double xValuePerPixel;
   double yValuePerPixel;
   //String name = "Plot Axis";
   
   boolean autoRange = false;   // TODO: not implemented
   
   public ArrayList<Plot2dSeries> series = new ArrayList<Plot2dSeries>();
   ...
   public Plot2dAxes()
   {
      ...
      //this.setBorder (BorderFactory.createTitledBorder ("Plot Border"));
      this.setBackground (Color.white);
      this.setOpaque (true);
      this.setName ("Plot Axes");
   }
   
   public Plot2dAxes(String name)
   {
       this.setBackground (Color.white);
       this.setOpaque (true);
       this.setName (name);
   ...
   public void paintComponent (Graphics g)
   {
      ...
      for (Plot2dSeries s : series)
      {
         g.setColor(s.color);
         int nPoints = s.getSize();
         int[] xI = new int[nPoints];
         int[] yI = new int[nPoints];
         
         for (int i=0; i<s.getSize (); i++)
         {
            xI[i] = (int)((s.x[i] - xLim[0])/xValuePerPixel);
            yI[i] = (int)(this.getHeight() - (s.y[i] - yLim[0])/yValuePerPixel);
         }
         
         if (s.drawLines == true)
         {
            g2.setStroke (new BasicStroke (s.lineWidth));
            g2.drawPolyline(xI, yI, nPoints);
         }
         if (s.drawPoints == true)
         {
            for (int i=0; i<s.getSize (); i++)
            {
               g.fillOval (xI[i] - 4, (int)yI[i] - 4, 8, 8);
            }
            //drawKnots (g);      // Draw Knot Points
         }
      }
...
//      {
//         g.drawRect ( zoomSelectRect.x, zoomSelectRect.y, zoomSelectRect.width, zoomSelectRect.height);
//      }

   }
   
   public void determineXValuePerPixel() 
   {
      xValuePerPixel = (xLim[1]-xLim[0])/getWidth();
   }

   public void determineYValuePerPixel()
   {
      yValuePerPixel = (yLim[1]-yLim[0])/getHeight();
   }
   
   
   
   private static double displayDigits = 8;
   private static double[] units = new double[] { 5, 2, 1 };
   private static int maxPixels = 45;
   
   
   private void drawGrids (Graphics g) {
      double yFactor = 1.0 / yValuePerPixel;
      double ySpacing = getSpacing (yValuePerPixel);
      double yPos = ySpacing * Math.ceil (yLim[0] / ySpacing);
      double xFactor = 1.0/xValuePerPixel;
      double xSpacing = getSpacing (xValuePerPixel);
      ...
      int round = Math.max (
         yPosStr.substring (yPosStr.indexOf (".") + 1).length (),
         ySpacingStr.substring (ySpacingStr.indexOf (".") + 1).length () ); // TODO rounding?
      
      for (int i = 0; yPos < yLim[1]; yPos += ySpacing, i++) 
      {
         int yi = (int)(-(yPos - yLim[1]) * yFactor);
      ...
      double xPos = 0;
      double xOffset = xSpacing * Math.ceil (xLim[0] / xSpacing);

//      xSpacing = xSpacing * myProbe.getScale();
//      xFactor = xFactor / myProbe.getScale();
      
      for (int i = 0; xPos < xLim[1]; xPos += xSpacing, i++) 
      {
   ...
   public void updateRangeFromData()
   {
      double[] xRange = {0.0, 1.0};
      double[] yRange = {0.0, 1.0};
      
      for (int i=0; i<series.size(); i++)
      {
         if (i==0)
         {
            xRange = series.get(i).getXRange();
            yRange = series.get(i).getYRange();
         }
         else
         {
            double[] xR = series.get(i).getXRange();
            double[] yR = series.get(i).getYRange();
            if (xR[0] < xRange[0])
               xRange[0] = xR[0];
            if (xR[1] > xRange[1])
               xRange[1] = xR[1];
   ...
   public void addSeries(Plot2dSeries series)
   {
      if (series.x==null)
      {
         series.x = new double[]{0.0, 1.0};
         series.y = new double[]{0.0, 1.0};
      }
      else if (series.y==null)
      {
         series.y = new double[series.x.length];
      }
      
      this.series.add (series);
   }
   
   public void addSeries(Plot2dSeries[] seriesArray)
   {
       for (Plot2dSeries series : seriesArray)
           addSeries(series);
   }
   
   public ArrayList<Plot2dSeries> getSeries()
   {
      return series;
   }
...
//      // to resolve a bug when probes are created with a display size of zero
...
//      setDefaultXRange (autoRange[0], autoRange[1]);
//   }
//
...
//    * @param max
//    */
//   public void setDefaultXRange (double min, double max) {
//      defaultRange[0] = min;
//      defaultRange[1] = max;
//   }
//
//   public double[] getDefaultXRange() {
//      return defaultRange;
//   }
//
//   public double[] getDefaultYRange() 
//   {
//      return yLim;
//   }
//
//   public void setDefaultDomain() {
//      double[] domain = getDefaultYRange();
...
//
//      setAutoRange();
//      double[] defaultDisplayRange = getDefaultXRange();
//      double[] defaultDomain = getDefaultYRange();
//
...
//      double[] range = series.get(0).getXRange ();      // TODO: not general...
//      setDisplayRange (range[0], range[1]);
//      repaint();
//   }
//
...
//      double[] defaultRange = getDefaultXRange();
//
//      double rangeSize =
...
//      // points
//      double[] maxDomain = getDefaultYRange();
...
//      // in or out
//      if (zoomLevel == 0) {
//         setDefaultXRange (newYMin, newYMax);
//      }
//   }
//
//   /**
...
//            if (!legend.isVisible()) {
//               GuiUtils.locateRight (legend, Plot2dAxes.this);
