...
**Optional:** `runSims.bash` starts one manager and several workers in the background (template). It already points at `FrankModel3`; adjust worker count and paths for your machine.
...
| `setUpStopConditionMonitor()` | frank3 | defines success | Sets `myMaxTime` / `mySettleTime` and adds an `EquilibriumChecker` requiring all enabled-FEM nodes to be static within the settle window ending at `myMaxTime`. Not settled in time → task fails. |
